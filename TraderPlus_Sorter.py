...
input_file = r"Path to your TraderPlusPriceConfig.json"
output_file = os.path.join(os.path.dirname(input_file), "TraderPlusPriceConfig_sorted.json")

with open(input_file, "r") as f:
    data = json.load(f)

def sort_by_buy_price(product):
    return int(product.split(',')[-2])
